...
X-ray offsets (`ymeas\_raw\_w - y\_corrangle` as labelled in the database) are used to create an "x-ray track." The offsets are used as the "positions" on each layer used to build a track. X-ray residuals are calculated using a track built from the offsets on two fixed layers. The x-ray residuals can then be compared to the mean of cosmics residuals around them,tabulated with the same reference layers. The program repeats this process for all positions on the quadruplet where x-ray data was taken for all possible sets of reference layers. Several outputs characterizing the cosmics data and comparing the cosmics and x-ray data are generated.
...
`cosmics_xray_correlation` and ReClustering (a program in the so-named subdirectory, described later) have the same dependencies as [`tgc_analysis`](https://gitlab.cern.ch/McGill-sTGC/tgc_analysis), detailed in its README. To setup the appropriate environment on LXPLUS, run the following:
...
input\_xray\_database.db is the x-ray database. The most recent x-ray database as of 2021-11-26 is stored in the subdirectory, xrayData.
...
ReClustering output is stored in lxplus.cern.ch:/eos/atlas/atlascerngroupdisk/det-nsw-stgc/cosmics\_test\_bench\_data/QUADTYPE/QUADNAME/ANALYSIS\_3100V/reclustering
...
- usefulScripts: As the name suggests. Includes a minimal script called testGeom.cpp to manipulate the `tgc_analysis` geometry object and some sample code on how to generate a sql database from a csv file.
...
Some of the strips on the long edge of a quadruplet are really noisy. A warning will be printed to stdout if this is the case (see L482 of ReClustering.cpp). If required to run `ReClustering` again, for the quads listed starting at [L328](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/ReClustering/ReClustering.cpp#L328) of ReClustering.cpp, uncomment [L335-349](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/ReClustering/ReClustering.cpp#L335-349) and adjust accordingingly to reject all clusters originally recorded at that top strip.
...
1. Get the x-ray data from xray\_results\_2021-11-26.db and store them in XRayData objects ([L97](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L97)).  
2. Create tracks from the x-ray offsets and calculate the x-ray residuals. The x-ray residuals are stored in the residuals member vector of the XRayRetracking class ([L103](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L103)).  
3. Retrack the cosmics data. Loop over every event stored in the `ReClustering` tracks TTree and rebuild the track for all possible sets of reference layers. Calculate residuals for all possible tracking combinations and store them in a vector of residuals ([L118](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L118)).  
4. Compare the cosmics and x-ray data ([L127](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L127)). For each x-ray residual, define a region of interest around it (with widths in x and y specified in the constructor). Fit the residual distribution for all residuals of the given tracking combination falling in the region of interest with a Gaussian. Store the x-ray residual and mean cosmics residuals in a LocalData object. The CompareData class keeps a vector of the LocalData objects. Put each x-ray and mean cosmics residual pair in a scatter plot to assess the correlation ([L128](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L128)). Output the LocalData object parameters to csv ([L129](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L129)).  
5. Create various plots from the cosmics data only using the ResPlots class ([L132-144](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsXRayCorrelation.cpp#L132-144)). Most important are the 2D histograms of residual means for each tracking combination over the surface of the quadruplet. These are visual indications of the relative misalignments between layers of the quadruplet.

The x-ray data is extracted from the database using a sqlite3 select statement defined on [L32-42](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/XRayData.cpp#L32-42) of XRayData.cpp. Edit the select statement to change what x-ray data gets selected. The quad the x-ray measurement was taken on is not included in the x-ray database. To select data from a paticular quadruplet, the select statement asks for entries on the correct wedge (appropriate MTF for the quad) and of the correct quad type. The entries returned are ordered by xnom to sort them by x-ray gun position.

In the language of the column names in the sql database, the x-ray $`offset = ymeas\_raw\_w - y\_corrangle`$. The uncertainty in the x-ray offset is 120 um ([L93](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/XRayData.cpp#L93)), as accepted by the working group.

The tracks in the input `ReClustering` tracks TTree are re-tracked with two reference layers in CosmicsRetracking.cpp. The uncertainty on the cluster is 60 um (assigned on [L54](https://gitlab.cern.ch/McGill-sTGC/cosmics_xray_correlation/-/blob/e35f9c0213c70b584e278ca3481a5397444bedfd/src/CosmicsRetracking.cpp#L54)) based on appendix A in [Lia's thesis](https://github.com/liaformenti/thesis). 
